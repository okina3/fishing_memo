...
class SpotTest extends TestCase
{
   ...
   // ピボット属性付きで釣り場にメモを関連付けるヘルパーメソッド
   private function attachMemos(Spot $spot, int $count): Collection
   {
      // メモを作成
   ...
   public function testSpotAttributesAndRelations()
   {
      ...
      // 釣り場に2件のメモを関連付け（ピボットデータ付き）
      $createdMemos = $this->attachMemos($spot, 2);

      // リレーションを最新化しておく（テストの安定化のため）
      $spot->load('memos', 'user');

      // 釣り場とメモのリレーションが、正しいインスタンスであることを確認
      $this->assertInstanceOf(BelongsToMany::class, $spot->memos());
      // メモのID配列が釣り場の関連IDと一致するか確認（順序非依存）
   ...
   {
      // 3件の釣り場を作成
      $spots = Spot::factory()->count(3)->create(['user_id' => $this->user->id]);
      // 全ての釣り場を取得
      $allSpots = Spot::availableAllSpots()->get();

      // 釣り場のID配列が取得結果と一致するか確認（順序非依存）
