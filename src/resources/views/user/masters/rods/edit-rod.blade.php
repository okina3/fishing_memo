<x-app-layout>
   ...
   <div class="px-2 py-2 bg-slate-200">
      <section class="text-gray-600 border border-gray-400 rounded-lg bg-white overflow-hidden">
         {{-- タイトル --}}
         <h1 class="heading heading_bg">釣り竿の編集</h1>
         <div class="p-3">
            {{-- フラッシュメッセージ --}}
            <x-common.flash-message status="session('status')" />
            {{-- 選択した釣り竿を編集するエリア --}}
            <form action="{{ route('user.rod.update') }}" method="POST">
               @csrf
               @method('patch')
               {{-- 現在の釣り竿名の表示 --}}
               <div>
                  <h2 class="sub_heading-2 mb-2 block">現在の釣り竿名</h2>
               ...
               <div class="mb-10">
                  {{-- 新しい釣り竿名（上書き） --}}
                  <label class="sub_heading mb-2 block">新しい釣り竿名（上書き）</label>
                  <input class="w-full rounded" name="rod_name" type="text"
                     value="{{ old('rod_name', $rod->name) }}">
                  {{-- エラーメッセージ（釣り竿の更新） --}}
            ...
            {{-- 戻るボタン --}}
            <x-user.button.back-button-masters :tab="'rods'" />
         </div>
      </section>
   </div>
</x-app-layout>
