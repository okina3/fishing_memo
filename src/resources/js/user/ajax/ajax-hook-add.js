...
document.addEventListener('DOMContentLoaded', () => {
   //追加ボタン、新規釣り針入力欄、釣り針セレクトボックスの要素の取得
   const addBtn = document.getElementById('add_hook_btn');
   const input = document.getElementById('hook_name_input');
   ...
   const messageEl = document.getElementById('hook_message');
   const clearMessage = () => {
      if (!messageEl) return;
   ...
   // 釣り針の追加の実行
   const addHook = async (newHook) => {
      const url = addBtn.dataset.url || '/hook/store-ajax';
      const headers = {
      ...
      try {
         ...
         if (res.status === 201) {
            ...
            input.value = '';
            showMessage('釣り針の選択肢に追加しました', 'success');
         ...
         if (res.status === 422) {
            ...
            const serverMsg = data?.errors?.hook_name?.[0] ?? data?.message;
         ...
         try {
            ...
            const otherMsg = otherData?.errors?.hook_name?.[0] ?? otherData?.message;
            // 通常のバリデーションではじかれた場合のエラーメッセージ（保険）。
   ...
   addBtn.addEventListener('click', () => {
      ...
      if (!newHook) {
         showMessage('釣り針名を入力してください。', 'error');
         return;
      }
      addHook(newHook);
   });
