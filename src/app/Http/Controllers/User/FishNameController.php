...
class FishNameController extends Controller
{
    ...
    * @throws Throwable
    */
   public function storeAjax(StoreFishRequest $request): JsonResponse
   {
      try {
   ...
   public function edit(int $id): View
   {
      ...
      return view('user.masters.partials.fish-names.edit-fish-name', compact('fish_name'));
   }
   ...
   public function update(StoreFishRequest $request): RedirectResponse
   {
      try {
         ...
         return to_route('user.masters.index', ['tab' => 'fishNames'])
            ->with(['message' => '魚名を更新しました。', 'status' => 'success']);
      } catch (Throwable $e) {
         Log::error($e);
         return back()->with(['message' => '魚名の更新に失敗しました。', 'status' => 'error']);
      }
   }
   ...
   public function destroy(Request $request): RedirectResponse
   {
      try {
         ...
         // 選択した魚名を削除
         $fish_name->delete();
         return redirect()->back()->with(['message' => '正常に魚名を削除しました。', 'status' => 'success']);
      } catch (Throwable $e) {
         Log::error($e);
         return redirect()->back()->with(['message' => '魚名の削除に失敗しました。', 'status' => 'error']);
      }
   }
}
